...
# -*- coding: utf-8 -*-
"""
wind_u10_v10_separate_nc_to_swat.py
------------------------------------------
...
It uses:
  • polygon–polygon FRACTIONAL OVERLAP (equal-area CRS) to weight grid cells by
    their area fraction intersecting the basin (your preferred method).
  • robust time alignment between u10 and v10 halves per year.
  • local-time grouping before daily averaging (important for hydrology).
  • basin-weighted daily mean series + monthly averages + monthly climatology.
  • simple diagnostics and plots.

Notes (with official references):
  • ERA5-Land provides u10 and v10 as instantaneous hourly components in m/s;
    wind speed |V| = sqrt(u10^2 + v10^2).  [ECMWF/ERA5-Land variable list]  [1](https://swat.tamu.edu/data/)
  • SWAT (WGN/observed inputs) expects daily wind speed in **m/s**.  
...
NC_DIR_U10 = r"C:/Users/Diwakar Adhikari/Downloads/SWAT+/ERA5_Land/Wind Speed/wind_u10"
NC_DIR_V10 = r"/Users/Diwakar Adhikari/Downloads/SWAT+/ERA5_Land/Wind Speed/wind_v10"

# Output directory
OUT_DIR = r"C:/Users/Diwakar Adhikari/Downloads/SWAT+/ERA5_Land/Wind Speed"
os.makedirs(OUT_DIR, exist_ok=True)

# Years & halves (per your request)
...
def compute_fractional_overlap_weights(lats, lons, basin_union_wgs84, basin_union_ea):
    ...
    basin_area_km2 = 0.0
    try:
        # if basin_union_ea is a single polygon
        basin_area_km2 = float(basin_union_ea.area / 1e6)
    except Exception:
        # if it's a MultiPolygon, fall back to gdf sum
        pass
...
def main():
    ...
    for year in tqdm(YEARS, desc="Years"):
        for hu, hv in zip(HALVES_u10, HALVES_v10):
            ...
            with netCDF4.Dataset(path_u, 'r') as dsU, netCDF4.Dataset(path_v, 'r') as dsV:
                ...
                df_hour = pd.DataFrame(speed_utc, index=idx_local)  # columns implicit 0..n_grid-1
    ...
    monthly_wide = monthly_df.pivot(index="Year", columns="Month",
                                    values="MonthlyMeanDailyWind_mps").sort_index()
